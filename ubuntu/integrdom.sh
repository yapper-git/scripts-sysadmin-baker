...
# - valeur de vérification 12.04 remplacée par 14.04 ainsi que 16.04 (+17/18 pour mint)
...
###########################################################################
scribe_def_ip="10.93.200.252"
proxy_def_ip="172.16.0.252"
proxy_def_port="3128"
proxy_gnome_noproxy="[ 'localhost', '127.0.0.0/8', '172.16.0.0/16' ]"
...
########################################################################
. /etc/lsb-release
if [ "$DISTRIB_RELEASE" != "14.04" ] && [ "$DISTRIB_RELEASE" != "17.3" ] && [ "$DISTRIB_RELEASE" != "16.04" ] && [ "$DISTRIB_RELEASE" != "18" ] ; then
  echo "Vous n'êtes pas sur une version compatible, rappel des versions supportées pour ce script : Ubuntu & Variante 14.04/16.04, Linux Mint 17.3/18"
  exit 1
fi
...
#numlockx pour le verrouillage du pave numérique
...
########################################################################
#paramétrage du script de démontage du netlogon pour lightdm # désactivé pour Mint
########################################################################
...
#activation du pave numérique par greeter-setup-script=/usr/bin/numlockx on
########################################################################
echo "[SeatDefaults]
    allow-guest=false
...
#paramétrage du launcher unity par défaut : nautilus, firefox, libreoffice, calculatrice, éditeur de texte et capture d'écran
########################################################################
...
favorites=[ 'nautilus-home.desktop', 'firefox.desktop', 'chromium-browser.desktop', 'libreoffice-writer.desktop', 'libreoffice-calc.desktop', 'libreoffice-impress.desktop', 'gedit.desktop' , 'edu.media.mit.scratch2editor.desktop' , 'gnome-calculator.desktop' ]
" > /usr/share/glib-2.0/schemas/my-defaults.gschema.override

  glib-compile-schemas /usr/share/glib-2.0/schemas

# Suppression de paquets inutiles sous Ubuntu/Unity
  apt-get -y purge aisleriot gnome-mahjongg

fi

# Pour être sûr que les paquets suivant (parfois présents) ne sont pas installés :
